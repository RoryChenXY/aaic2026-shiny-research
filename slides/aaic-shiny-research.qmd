...
-   1:00 - 1:30 p.m. - troductions: welcome remarks, goals, facilitator and participant introductions, background and rearrange in groups
(Ana Capuano)
...
## 1.3 Workshop Goal {.smaller}

This hands-on workshop will help dementia researchers design, build, and share interactive dashboards using R Shiny.

**By the end of the workshop, participants will be able to:**

-   Transform static research outputs into interactive, transparent, and reproducible data tools.
-   Build a working Shiny dashboard using openly available GBD Alzheimer’s disease and other dementias data.
-   Apply core Shiny concepts.
-   Explore real-world dementia research examples, including the DPAU Metadata Explorer app.

------------------------------------------------------------------------



## 1.4 Why use `shiny` for your research {.smaller}

R Shiny allows researchers to turn analyses, figures, tables, and statistical results into interactive tools that collaborators and stakeholders can explore.

**Advantages of R Shiny**

-   Creates interactive dashboards directly from R.
-   Allows users to explore data without editing code.
-   Combines figures, tables, maps, models, and text in one interface.
-   Makes research outputs easier to share and interpret.
-   Supports transparent and reproducible workflows.
-   Helps collaborators interact with results instead of receiving static files.
-   Can be used for teaching, communication, team science, and knowledge translation.


------------------------------------------------------------------------

## 1.5 Examples of R Shiny in Research

-   Posit Shiny Gallery   <https://shiny.posit.co/r/gallery/#feature-demos>
-   Epidemiologist R Handbook: Shiny dashboards - <https://epirhandbook.com/en/>



------------------------------------------------------------------------

### 1.5.1 Examples of R Shiny in Research

![`Examples`](./img/s1.5.1-shinyexample.png)

## 1.6 Let’s Learn Who Is in the Room
...
-   Your name
-   Your affiliation or association
-   The type of research you do
-   The type of data you work with
-   Your prior experience with R or R Shiny


`Purpose`: This will help us understand the range of backgrounds in the room and organize groups for the hands-on portion.
...
-   Please make sure you have access to the workshop materials and have the required software installed.
...
-   Participants with prior R Shiny experience should sit near participants who are new to Shiny.
-   The goal is peer support and shared learning.


------------------------------------------------------------------------


###  1.6.2 Setup - Projects
...
![`New Project`](./img/s1.6.1-newproject.png)

------------------------------------------------------------------------

###  1.6.3 Setup - Packages
...
###  1.6.4 Setup - API keys (**Optional**)
-   Please open the Google Sheet, add your name and email address to the list, and copy your API key.
-   Please only take one.
-   If you have trouble, please raise your hand, one of the facilitators will come to assist you.

![`API Keys`](./img/qrcode.png)

------------------------------------------------------------------------

###  1.6.4 Setup - API keys (**Optional**)

-   Run: `usethis::edit_r_environ()` in the console to open your `.Renviron` file.

-   In the `.Renviron` file, add the following line, replacing `your_api_key_here` with your actual API key: `ANTHROPIC_API_KEY="your_api_key_here"`

-   Restart R Studio to make sure the API keys are loaded. 
-   You can check if the key is loaded by running `Sys.getenv("ANTHROPIC_API_KEY")` in the console. 
...
-   `Shiny` package has `11` built-in examples.Each example is a self-contained Shiny app. You can run these examples to see how different Shiny features work in practice.
...
-   Hello Shiny example: Open and run `./examples/03-helloshiny.R`
...
-  You can find the solution in `./examples/solution/04-helloaaic-solution.R`
...
### 2.2.1 Shiny UI - Inputs {.small-contd}
...
-   Open `./examples/exercises/06-uitables.R`
    -   Add a `sliderInput()` to control how many rows are displayed in `tableOutput`
    -   Add `tableOutput()` with `renderTable()`
    -   Add `dataTableOutput()` with `renderDataTable()`
...
-   You can find the solution in `./examples/solution/06-uitables-solution.R`
...
-   For this exercise, we will add a Plot tab to the previous example. (You can also refer to `03-helloshiny.R` for how to create a histogram.)
-   UI:
    -  In `sidebar()`, add a new `sliderInput` for number of bins.
    -  In `navset_card_underline()`, add a new `nav_panel()`, and place a `plotOutput()` inside it.
-   Server:
    -  Add a new `renderPlot()` to generate the histogram of `p_tau` in the `ad_data` dataset, using the number of bins specified by the new slider input.
    -  Optional goal: Try different plot types. 
-  You can find the solution in `./examples/solution/07-uitabs-solution.R`
...
-   When user changes an input, the server automatically re-runs the relevant code to update the outputs. This is called `reactivity`.

-    How to do this?
     -  1. Add an R object to user interface.
     -  2. Define the output in the server function, wrapped in a call to `render*()` to indicate that it is reactive and specify its output type. 
...
-  You can find the solution in `./examples/solution/08_reactiveexp-solution.R`
...
### 3.1.1 DataRepExp - Recording

<video width="1000" height="600" controls>
  <source src="./img/datarepexp.mp4" type="video/mp4">
</video>
...
### 3.2.1 GLADSE Recording (DPAU Explorer)


<video width="1000" height="600" controls>
  <source src="./img/gladse.mp4" type="video/mp4">
</video>
...
![`Survey Evaluation`](./img/evaluation_QR.png)
